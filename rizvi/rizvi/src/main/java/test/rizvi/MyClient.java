...
//import com.hortonworks.simpleyarnapp.Client;
...
public class MyClient {
	...
	public void run(String[] args) throws Exception {
		...
		amContainer.setCommands(Collections.singletonList("$JAVA_HOME/bin/java"
				+ " -Xmx256M"
				+ " test.rizvi.RizApplicationMaster" 
				+ " " + command
				+ " " + scriptPath.toString()
				+ " " + String.valueOf(n) 
				+ " 1>>" + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/am_stdout"
				+ " 2>>" + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/am_stderr"));
		...
		setupScriptLocalResource(scriptPath, scriptLocalResource);
		
		//amContainer.setLocalResources(Collections.singletonMap("rizviapp.jar",appMasterJar));		
		Map<String, LocalResource> localRes = new HashMap<String, LocalResource>();
		localRes.put("rizviapp.jar",appMasterJar);
		localRes.put("script_lnk_am.sh", scriptLocalResource);
		amContainer.setLocalResources(localRes);
		...
		appContext.setApplicationName("rizvi"); // application name
		...
		System.out.println("CLIENT>>>Application " + appId + " finished with"
	...
	private void setupScriptLocalResource(Path scriptPath, LocalResource scriptLocalResource)
			throws IOException {
